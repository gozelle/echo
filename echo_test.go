...
func TestEcho_OnAddRouteHandler(t *testing.T) {
	...
	assert.Equal(t, Route{Method: http.MethodGet, Path: "/static", Name: "github.com/gozelle/echo.glob..func1"}, added[0].route)
	assert.Len(t, added[0].middleware, 0)
	
	assert.Equal(t, "domain.site", added[1].host)
	assert.Equal(t, Route{Method: http.MethodGet, Path: "/static/*", Name: "github.com/gozelle/echo.TestEcho_OnAddRouteHandler.func1"}, added[1].route)
